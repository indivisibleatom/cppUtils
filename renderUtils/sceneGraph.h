...
// TODO msati: move out Interaction from SceneNode to IInteractableSceneNode, or
// get rid
class ISceneNode : public IInteractable {
 public:
  ...
  std::vector<std::unique_ptr<ISceneNode>>::iterator childBegin() {
    return m_pChildren.begin();
  }
  std::vector<std::unique_ptr<ISceneNode>>::iterator childEnd() {
  ...
  int onMouseDragged(int deltaX, int deltaY,
                     const IKeyboardState& keyboardState) override;
...
class TransformationNode : public ISceneNode {
 public:
  ...
  void rotate(const Vector<float>& axis, float angle) {
    m_matrix.rotate(axis, angle);
  }
  void translate(const Vector<float>& translation) {
...
class CameraNode : public ISceneNode {
 ...
 private:
  int onMouseFreeDragged(int deltaX, int deltaY,
                         const IKeyboardState& keyboardState);
...
class LightNode : public ISceneNode {
 public:
  LightNode(const Point<float>& lightPosition)
