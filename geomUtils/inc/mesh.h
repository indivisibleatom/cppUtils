...
class IInteractableMesh : public ICommandable,
...
class ColorMap {
 ...
 public:
  ColorMap(std::unique_ptr<std::vector<COLORS>>& pColorMap) throw()
      : m_pColorMap(std::move(pColorMap)) {
    assert(c_numTriangleMarkers == m_pColorMap->size());
  }

  const COLORS& operator[](unsigned char index) const {
  ...
  unsigned int getIndexForColor(COLORS color) {
    ...
    return (iter == m_pColorMap->end()
                ? addColor(color)
                : std::distance(m_pColorMap->begin(), iter));
  }
  unsigned int addColor(COLORS color) {
    m_pColorMap->push_back(color);
    return m_pColorMap->size();
  }
};
...
class Mesh : public IInteractableMesh {
 ...
 protected:
  ...
  unsigned char m_selectedCornerPrevTM;  // For the color of the selected corner
 ...
 protected:
  ...
  virtual void setColorMap() {
    COLORS colorMap[] = {COLORS::RED,   COLORS::GREEN,   COLORS::BLUE,
                         COLORS::CYAN,  COLORS::MAGENTA, COLORS::YELLOW,
                         COLORS::BLACK, COLORS::WHITE,   COLORS::RED,
                         COLORS::NONE};
    std::unique_ptr<std::vector<COLORS>> pVecColorMap(
        new std::vector<COLORS>());
...
#pragma region InitMesh
  Point<U> centerBoundingBox() const throw();
  void init() override;
  void resetMarkers();
...
#pragma region CornerOperators
  TIndex t(CIndex c) const throw();
  CIndex n(CIndex c) const throw();
  CIndex p(CIndex c) const throw();
  VIndex v(CIndex c) const throw();
  CIndex o(CIndex c) const throw();
  CIndex l(CIndex c) const throw();
  CIndex r(CIndex c) const throw();
  CIndex s(CIndex c) const throw();
  CIndex u(CIndex c) const throw();
  CIndex c(TIndex t) const throw();
  CIndex offset(CIndex c) const throw();
  const Point<U>& g(CIndex c) const throw();
  const Point<U>& geom(VIndex v) const throw();
  CIndex c(TIndex tIndex, VIndex vIndex) const throw();

#pragma region MiscHelpers
  Point<U> centroid(TIndex t) const throw();

  VIndex nv() const throw();
  CIndex nc() const throw();
  TIndex nt() const throw();
  CIndex selectedCorner() const throw();
...
#pragma region SwingIterator
  class Swing_iterator
      : public std::iterator<std::input_iterator_tag, CIndex, ptrdiff_t,
                             const CIndex*,
                             const CIndex&>  // Info about iterator
        {
   private:
    CIndex m_corner;
   ...
   public:
    Swing_iterator(const Mesh<T, U>* const pMesh, CIndex corner,
                   bool fDoneAtleastOneSwing = false);
    const CIndex& operator*() const throw();
    const CIndex* operator->() const throw();
    ...
    bool operator!=(const Swing_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Swing_iterator& other) const throw() {
      return equals(other);
   ...
   private:
    bool equals(const Swing_iterator& other) const throw() {
      return (m_pMesh == other.m_pMesh && m_corner == other.m_corner &&
              m_fDoneAtleastOneSwing == other.m_fDoneAtleastOneSwing);
    }
  };

  Swing_iterator beginSwingIterator(CIndex corner) {
    return Swing_iterator(this, corner);
  }
  Swing_iterator endSwingIterator(CIndex corner) {
    return Swing_iterator(this, corner, true /*fDoneAtleastOneSwing*/);
  }
  Swing_iterator cBeginSwingIterator(CIndex corner) const {
    return Swing_iterator(this, corner);
  }
  Swing_iterator cEndSwingIterator(CIndex corner) const {
    return Swing_iterator(this, corner, true /*fDoneAtleastOneNext*/);
  }
...
#pragma region NextIterator
  class Next_iterator
   ...
   public:
    Next_iterator(const Mesh<T, U>* pMesh, CIndex corner,
                  bool fDoneAtleastOneNext = false)
        : m_pMesh(pMesh),
          m_corner(corner),
    ...
    bool operator!=(const Next_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Next_iterator& other) const throw() {
      return equals(other);
  ...
  Next_iterator beginNextIterator(CIndex corner) {
    return Next_iterator(this, corner);
  }
  Next_iterator endNextIterator(CIndex corner) {
    return Next_iterator(this, corner, true /*fDoneAtleastOneNext*/);
  }
  Next_iterator cBeginNextIterator(CIndex corner) const {
    return Next_iterator(this, corner);
  }
  Next_iterator cEndNextIterator(CIndex corner) const {
    return Next_iterator(this, corner, true /*fDoneAtleastOneNext*/);
  }
  Next_iterator beginNextIteratorWithT(TIndex triangle) {
    return Next_iterator(this, c(triangle));
  }
  Next_iterator endNextIteratorWithT(TIndex triangle) {
    return Next_iterator(this, c(triangle));
  }
...
#pragma region PrevIterator
  class Prev_iterator
   ...
   public:
    Prev_iterator(const Mesh<T, U>* pMesh, CIndex corner,
                  bool fDoneAtleastOneNext = false)
        : m_pMesh(pMesh),
          m_corner(corner),
          m_fDoneAtleastOneNext(fDoneAtleastOneNext) {}
    ...
    bool operator!=(const Prev_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Prev_iterator& other) const throw() {
      return equals(other);
  ...
  Prev_iterator beginPrevIterator(CIndex corner) {
    return Prev_iterator(this, corner);
  }
  Prev_iterator endPrevIterator(CIndex corner) {
    return Prev_iterator(this, corner, true /*fDoneAtleastOneNext*/);
  }
  Prev_iterator cBeginPrevIterator(CIndex corner) const {
    return Prev_iterator(this, corner);
  }
  Prev_iterator cEndPrevIterator(CIndex corner) const {
    return Prev_iterator(this, corner, true /*fDoneAtleastOneNext*/);
  }
...
#pragma region TriangleIterator
  class Triangle_iterator
      : public std::iterator<std::input_iterator_tag, TIndex, ptrdiff_t,
                             const TIndex*,
                             const TIndex&>  // Info about iterator
        {
   private:
    TIndex m_triangle;
    const Mesh<T, U>* m_pMesh;
   ...
   public:
    Triangle_iterator(const Mesh<T, U>* const pMesh, TIndex triangle)
        : m_pMesh(pMesh), m_triangle(triangle) {}
    ...
    bool operator!=(const Triangle_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Triangle_iterator& other) const throw() {
      return equals(other);
  ...
  Triangle_iterator beginTriangleIterator() {
    return Triangle_iterator(this, TIndex(0));
  }
  Triangle_iterator endTriangleIterator() {
    return Triangle_iterator(this, m_nt);
  }
  Triangle_iterator cBeginTriangleIterator() const {
    return Triangle_iterator(this, TIndex(0));
  }
  Triangle_iterator cEndTriangleIterator() const {
    return Triangle_iterator(this, m_nt);
  }
#pragma endregion TriangleIterator

#pragma region CornerIterator
  class Corner_iterator
      : public std::iterator<std::input_iterator_tag, CIndex, ptrdiff_t,
                             const CIndex*,
                             const CIndex&>  // Info about iterator
        {
   ...
   public:
    Corner_iterator(const Mesh<T, U>* pMesh, CIndex corner)
        : m_pMesh(pMesh), m_corner(corner) {}
    const CIndex& operator*() const throw() { return m_corner; }
    const CIndex* operator->() const throw() { return &m_corner; }
    ...
    bool operator!=(const Corner_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Corner_iterator& other) const throw() {
      return equals(other);
  ...
  Corner_iterator beginCornerIterator() const {
    return Corner_iterator(this, CIndex(0));
  }
  Corner_iterator endCornerIterator() const {
    return Corner_iterator(this, m_nc);
  }
  Corner_iterator cBeginCornerIterator() const {
    return Corner_iterator(this, CIndex(0));
  }
  Corner_iterator cEndCornerIterator() const {
    return Corner_iterator(this, m_nc);
  }
...
#pragma region VertexIterator
  class Vertex_iterator
      : public std::iterator<std::input_iterator_tag, VIndex, ptrdiff_t,
                             const VIndex*,
                             const VIndex&>  // Info about iterator
        {
   private:
    VIndex m_vertex;
    const Mesh<T, U>* const m_pMesh;
   ...
   public:
    Vertex_iterator(const Mesh<T, U>* const pMesh, VIndex vertex)
        : m_pMesh(pMesh), m_vertex(vertex) {}
    ...
    bool operator!=(const Vertex_iterator& other) const throw() {
      return !equals(other);
    }
    bool operator==(const Vertex_iterator& other) const throw() {
      return equals(other);
  ...
  Vertex_iterator beginVertexIterator() {
    return Vertex_iterator(this, VIndex(0));
  }
  Vertex_iterator endVertexIterator() { return Vertex_iterator(this, m_nv); }
  Vertex_iterator cBeginVertexIterator() const {
    return Vertex_iterator(this, VIndex(0));
  }
  Vertex_iterator cEndVertexIterator() const {
    return Vertex_iterator(this, m_nv);
  }
#pragma endregion VertexIterator
  ...
  void computeO() {
    ...
    for (VIndex i = VIndex(0); i < m_nv; i++) {
      valence[i] = CIndex(0);  // valences wil be reused to track how many
                               // incident corners were encountered for each
                               // vertex
    }

    std::vector<CIndex> C(m_nc);
    std::for_each(
        beginCornerIterator(), endCornerIterator(),
        [this, &C, &firstIncidentCorner, &valence](const CIndex& corner) {
          C[firstIncidentCorner[v(corner)] + valence[v(corner)]++] =
              corner;  // for each vertex: the list of val[v] incident corners
                       // starts at C[fic[v]]
          m_OTable[corner] = -1;
        });

    for (VIndex i = VIndex(0); i < m_nv; i++) {
      for (CIndex c1 = firstIncidentCorner[i];
           c1 < firstIncidentCorner[i] + valence[i] - 1; c1++) {
        for (CIndex c2 = CIndex(c1 + 1);
             c2 < firstIncidentCorner[i] + valence[i]; c2++) {
          // for each pair (C[a],C[b[]) of its incident corners
          if (v(n(C[c1])) == v(p(C[c2]))) {
            m_OTable[p(C[c1])] = n(C[c2]);
            m_OTable[n(C[c2])] = p(C[c1]);
          }
          // if C[a] follows C[b] around v, then p(C[a]) and n(C[b]) are
          // opposite
  ...
  Vector<U> triangleNormal(CIndex corner, bool fNormalized) const {
    // Vector<U> normal = Vector<U>( g(corner), g( p( corner )
    // )).cross(Vector<U>( g( corner ),g( n( corner ) ) ) );
    Vector<U> normal = Vector<U>(g(corner), g(n(corner)))
                           .cross(Vector<U>(g(corner), g(p(corner))));
    return fNormalized ? normal.normalize() : normal;
  }

  void populateNormals() {
    std::for_each(
        cBeginVertexIterator(), cEndVertexIterator(),
        [this](VIndex vIndex) { m_normals.push_back(Vector<float>(0, 0, 0)); });

    std::for_each(cBeginCornerIterator(), cEndCornerIterator(),
                  [this](CIndex cIndex) {
                    m_normals[v(cIndex)].add(triangleNormal(cIndex, false));
                  });

    std::for_each(cBeginVertexIterator(), cEndVertexIterator(),
                  [this](VIndex vIndex) { m_normals[vIndex].normalize(); });
  }

  Vector<U> vNormal(CIndex corner) {
    return m_normals[v(corner)];
    /*Vector<U> normal;
    std::for_each( beginSwingIterator( corner ), endSwingIterator( corner ), [
    this, &normal ] ( CIndex cIndex ) {
  ...
  void computeBox() {
    ...
    std::for_each(
        cBeginVertexIterator(), cEndVertexIterator(),
        [this, &highBox, &lowBox](const VIndex& vIndex) {
  ...
  void scaleMesh(float desiredBoundingBoxSize) {
    float boundingBoxSize =
        std::max<float>(m_boundingBox.high().x() - m_boundingBox.low().x(),
                        m_boundingBox.high().y() - m_boundingBox.low().y());
    boundingBoxSize = std::max<float>(
        m_boundingBox.high().z() - m_boundingBox.low().z(), boundingBoxSize);
    float scale = desiredBoundingBoxSize / boundingBoxSize;
    std::for_each(cBeginVertexIterator(), cEndVertexIterator(),
                  [this, &scale](VIndex vIndex) {
                    m_GTable[vIndex].set(scale * m_GTable[vIndex].x(),
                                         scale * m_GTable[vIndex].y(),
                                         scale * m_GTable[vIndex].z());
                  });
    computeBox();
  }
  ...
  bool isValenceBounded(CIndex cIndex, int maxValence) {
    int valence = 0;
    for (Swing_iterator iter = beginSwingIterator(cIndex);
         iter != endSwingIterator(cIndex); iter++) {
      valence++;
  ...
  void checkMaxValence() {
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this](const CIndex& cIndex) {
                    if (!isValenceBounded(cIndex, MAX_VALENCE)) {
                      std::cout << "Error " << cIndex;
                      LOG("Incorrect Mesh as valence > MAX_VALENCE ",
                          DEBUG_LEVELS::LOW);
                    }
                  });
  }
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this](const CIndex& cIndex) {
                    ...
                    if (o(o(cIndex)) != cIndex) {
                      std::cout << "Error o consistency" << cIndex << " "
                                << o(cIndex);
                      return;
                    }
                  });
...
#pragma region LOADING AND SAVING
  ...
  void saveMeshVTS(const std::string& fileName) {
    ...
    std::for_each(
        m_GTable.begin(), m_GTable.end(), [&file](const Point<U>& point) {
    ...
    std::for_each(m_VTable.begin(), m_VTable.end(),
                  [&file, &count](const VIndex& vIndex) {
                    file << vIndex;
                    count++;
                    file << (((count % 3) == 0) ? "\n" : ",");
                  });
  ...
  void quantizeGeometry(int numBits,
                        std::vector<Point<int>>& quantizedGeometry) {
    std::for_each(m_GTable.begin(), m_GTable.end(),
                  [&quantizedGeometry, this, &numBits](const Point<U>& point) {
                    quantizedGeometry.push_back(point.quantizePoint<int>(
                        m_boundingBox.low(), m_boundingBox.high(), numBits));
                  });
  }
 ...
 public:
  void serializeMeshVTS(const std::string& fileName) {
    ...
    std::for_each(beginVertexIterator(), endVertexIterator(),
                  [this, &file](VIndex vIndex) {
  ...
  void deserializeVTS(const std::string& fileName) {
    ...
    std::for_each(
        beginVertexIterator(), endVertexIterator(),
        [this, &file](VIndex vIndex) { deserializeVertexData(vIndex, file); });
...
#pragma region DISPLAY
  ...
  void draw() override {
    ...
    if (m_fDrawPlane) {
      ...
      glVertex3f(m_boundingBox.low().x() - 10000,
                 m_boundingBox.low().y() - 10000, m_boundingBox.low().z());
      glVertex3f(m_boundingBox.low().x() - 10000,
                 m_boundingBox.high().y() + 10000, m_boundingBox.low().z());
      glVertex3f(m_boundingBox.high().x() + 10000,
                 m_boundingBox.high().y() + 10000, m_boundingBox.low().z());
      ...
      std::for_each(cBeginCornerIterator(), cEndCornerIterator(),
                    [this](const CIndex& cIndex) {
                      if (displayCorner(cIndex)) {
                        uint32_t value =
                            (unsigned int)(*(m_pColorMap))[m_cm[cIndex]];
    ...
    if (m_fShowVertices) {
      std::for_each(cBeginVertexIterator(), cEndVertexIterator(),
                    [this](const VIndex& vIndex) {
                      if (displayVertex(vIndex)) {
                        uint32_t value =
                            (unsigned int)(*(m_pColorMap))[m_vm[vIndex]];
                        uint8_t b = (value >> 8) & 0xFF;
                        uint8_t g = (value >> 16) & 0xFF;
                        uint8_t r = (value >> 24) & 0xFF;
                        glColor3ub(r, g, b);
  ...
  void updateColorsVBO() {
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &col](const CIndex& cIndex) {
                    uint32_t value =
                        (unsigned int)(*(m_pColorMap))[m_tm[t(cIndex)]];
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_colorVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(uint8_t) * col.size(), col.data(),
                 GL_DYNAMIC_DRAW);
  ...
  void updateGeometryVBO(int typeMesh = 0)  // 0 static, 1 dynamic
  {
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &geometry](const CIndex& corner) {
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &normals](const CIndex& corner) {
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &edgeGeometry](const CIndex& corner) {
                    const Point<U>& currentPoint = g(corner);
                    ...
                    const Point<U>& nextPoint =
                        g(CIndex(corner + offsetForNext));
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_vertexVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(U) * geometry.size(), geometry.data(),
                 typeMesh == 0 ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, m_normalVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(U) * normals.size(), normals.data(),
                 typeMesh == 0 ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, m_edgeVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(U) * edgeGeometry.size(),
                 edgeGeometry.data(),
                 typeMesh == 0 ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
...
#pragma region HELPERS
  int getValence(CIndex corner) const {
    int valence = 0;
    std::for_each(cBeginSwingIterator(corner), cEndSwingIterator(corner),
                  [&valence](const CIndex& /*cIndex*/) { valence++; });
    return valence;
  }
...
#pragma region CONTROL
  int onMousePressed(int mouseX, int mouseY,
                     const IKeyboardState& keyboardState) override {
    return 0;
  }
  int onMouseReleased(int mouseX, int mouseY,
                      const IKeyboardState& keyboardState) override {
    return 0;
  }
  int onMouseDragged(int mouseX, int mouseY,
                     const IKeyboardState& keyboardState) override {
    return 0;
  }
  int onMouseMoved(int deltaX, int deltaY,
                   const IKeyboardState& keyboardState) override {
    return 0;
  }
  ...
  // TODO msati3: This has a dependency on FLTK right now. Can we remove this
  // without too much effort?
  int onKeyPressed(int ch) override {
    bool fHandled = true;
    switch (ch) {
      case 'h':
        selectCorner(unprojectPoint(Fl::event_x(), Fl::event_y()));
        break;
      case 'n':
        setSelectedCorner(n(m_selectedCorner));
        break;
      case 'p':
        setSelectedCorner(p(m_selectedCorner));
        break;
      case 'o':
        setSelectedCorner(o(m_selectedCorner));
        break;
      case 'l':
        setSelectedCorner(l(m_selectedCorner));
        break;
      case 'r':
        setSelectedCorner(r(m_selectedCorner));
        break;
      case 's':
        setSelectedCorner(s(m_selectedCorner));
        break;
      case 'u':
        setSelectedCorner(u(m_selectedCorner));
        break;
      case 'v':
        m_fShowVertices = !m_fShowVertices;
        break;
      case 'c':
        m_fShowCorners = !m_fShowCorners;
        if (!m_fShowCorners) {
          m_cm.resize(0);
          m_cm.shrink_to_fit();
        }
...
#pragma region CONTROL_HELPERS
 private:
  void selectCorner(const Point<U> point) {
    CIndex candidateNewCorner =
        m_selectedCorner == -1 ? CIndex(0) : m_selectedCorner;
    U minDistance = std::numeric_limits<U>::max();
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &point, &candidateNewCorner, &minDistance](
                      const CIndex& cIndex) {
    ...
    logStatement << "Corner picked : " << m_selectedCorner
                 << "  vertex: " << v(m_selectedCorner) << " vertex location "
                 << m_GTable[v(m_selectedCorner)];
  ...
  int interpretCommand(const std::string& command) {
    ...
    switch (command[0]) {
      case 'c':
        setSelectedCorner((CIndex)(strtoT<T>(command.c_str() + 1, nullptr)));
        break;
      case 'v': {
        VIndex vertex = (VIndex)strtoT<T>(command.c_str() + 1, nullptr);
        auto iter = std::find_if(
            beginCornerIterator(), endCornerIterator(),
            [this, &vertex](CIndex index) { return (v(index) == vertex); });
...
#pragma region MESH_ALGORITHMS
 private:
  ...
  void removeTriangle(CIndex corner) {
    ...
    if (toTIndex != fromTIndex) {
      ...
      std::for_each(beginNextIterator(initToCIndex),
                    endNextIterator(initToCIndex),
                    [this, &fromCIndexIterator](const CIndex& toCIndex) {
  ...
  void compressVTable() {
    ...
    // Compress the m_GTable, populate mapping from V index to compressed V
    // index
    std::for_each(beginVertexIterator(), endVertexIterator(),
                  [this, &newIndex, &vToCompressedVMap](const VIndex& vIndex) {
    ...
    // Use the mapping populated above to change the V table in accordance with
    // the new indices
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &vToCompressedVMap](const CIndex& cIndex) {
 ...
 public:
  ...
  // Make it so that this cIndex is moved to c( t ( cIndex ) ) -- cyclically
  // rotate
  // cIndex => 0, (cIndex+1)%3 => 1, (cIndex+2)%3 => 2
  // This is a cyclic permutation => cIndex -> 0, 0 -> offset, offset -> (offset
  // + offset)%3, etc
  void shiftTriangleCorners(CIndex cNewStartIndex) {
    ...
    std::for_each(
        beginNextIterator(initCorner), endNextIterator(initCorner),
        [this](const CIndex& cIndex) { m_OTable[m_OTable[cIndex]] = cIndex; });
  }

  void triangleExpandOperation(const std::vector<CIndex>& expansionCorners,
                               const std::vector<Point<U>>& expandedGeometry,
                               std::vector<VIndex>& addedVIndices,
                               std::vector<TIndex>& addedTIndices) {
    expandVertex(expansionCorners[1], expansionCorners[2], expandedGeometry[1],
                 expandedGeometry[2], true);
    expandVertex(expansionCorners[0], s(expansionCorners[1]),
                 expandedGeometry[0], expandedGeometry[1], false);
  ...
  // Expand the vertex given by corner1 and corner2. After expanding, corners
  // from s(corner1) to corner2 are incident on geom2. The rest on geom1
  VIndex expandVertex(CIndex corner1, CIndex corner2, const Point<U>& geom1,
                      const Point<U>& geom2, bool fAddTriangles) {
    assert(v(corner1) == v(corner2));
    replaceVertex(v(corner1), geom1);
    addVertex(geom2);

    for (auto iter = beginSwingIterator(s(corner1));
         iter != endSwingIterator(s(corner2)); iter++) {
  ...
  class LRTriangleIndexChangeHandler {
   ...
   public:
    LRTriangleIndexChangeHandler(TIndex lTriangle, TIndex rTriangle)
        : m_lTriangle(lTriangle), m_rTriangle(rTriangle) {}
  ...
  VIndex collapseTriangle(CIndex corner, const Point<U>& pointAfterCollapse) {
    ...
    std::function<void(typename Mesh<T, U>::TIndex&,
                       typename Mesh<T, U>::TIndex&)>
        triangleIndexChangeCallback(
            std::bind(&LRTriangleIndexChangeHandler::onTriangleIndexChanged,
                      &lrTriangleChangeHandler, std::tr1::placeholders::_1,
                      std::tr1::placeholders::_2));
    auto iterTriangleIndexChangeCallback =
        registerForTriangleMoveOperation(triangleIndexChangeCallback);

    Mesh<T, U>::VIndex simplifiedVIndex =
        collapseEdge(corner, opposite, pointAfterCollapse);
    unregisterForTriangleMoveOperation(iterTriangleIndexChangeCallback);
    simplifiedVIndex = collapseEdge(
        Mesh<T, U>::CIndex(c(lrTriangleChangeHandler.lTriangle()) + cLOffset),
        Mesh<T, U>::CIndex(c(lrTriangleChangeHandler.rTriangle()) + cROffset),
        pointAfterCollapse);

    return simplifiedVIndex;
  }

  VIndex collapseEdge(CIndex corner, CIndex oppositeCorner,
                      const Point<U>& pointAfterCollapse) {
    ...
    std::for_each(
        beginSwingIterator(cEdge2), endSwingIterator(cEdge2),
        [this, &vEdge1](const CIndex& cIndex) { setVTable(cIndex, vEdge1); });
    ...
    // Invalidate the o table of the triangle that is to be removed later. This
    // prevents overwriting of m_oppositeTable of the opposites when the
    // oppositeCorner triangle is the last triangle
    std::array<CIndex, 2> corners = {corner, oppositeCorner};
    std::for_each(corners.begin(), corners.end(), [this](const CIndex& corner) {
      std::for_each(
          beginNextIterator(corner), endNextIterator(corner),
          [this](const CIndex& cIndex) { setOTable(cIndex, CIndex(-1)); });
    });
...
#pragma region NOTIFICATIONS
 public:
  typename std::list<std::function<void(CIndex&, CIndex&)>>::const_iterator
  registerForCornerSelectionChangeOperation(
      std::function<void(CIndex&, CIndex&)> function) {
    return m_selectedCornerChangeOperationNotifiers.insert(
        m_selectedCornerChangeOperationNotifiers.end(), function);
  }

  void unregisterForCornerSelectionChangeOperation(typename std::list<
      std::function<void(CIndex&, CIndex&)>>::const_iterator& functionIter) {
    m_selectedCornerChangeOperationNotifiers.erase(functionIter);
  }

  typename std::list<std::function<void(VIndex&, VIndex&)>>::const_iterator
  registerForVertexMoveOperation(
      std::function<void(VIndex&, VIndex&)> function) {
    return m_vertexMoveOperationNotifiers.insert(
        m_vertexMoveOperationNotifiers.end(), function);
  }

  void unregisterForVertexMoveOperation(typename std::list<
      std::function<void(VIndex&, VIndex&)>>::const_iterator& functionIter) {
    m_vertexMoveOperationNotifiers.erase(functionIter);
  }

  typename std::list<std::function<void(TIndex&, TIndex&)>>::const_iterator
  registerForTriangleMoveOperation(
      std::function<void(TIndex&, TIndex&)> function) {
    return m_triangleMoveOperationNotifiers.insert(
        m_triangleMoveOperationNotifiers.end(), function);
  }

  void unregisterForTriangleMoveOperation(typename std::list<
      std::function<void(TIndex&, TIndex&)>>::const_iterator& functionIter) {
 ...
 private:
  void notifyVIndexChange(VIndex oldIndex, VIndex newIndex) {
    std::for_each(
        m_vertexMoveOperationNotifiers.begin(),
        m_vertexMoveOperationNotifiers.end(),
        [&oldIndex, &newIndex](std::function<void(VIndex&, VIndex&)> function) {
  ...
  void notifyTIndexChange(TIndex oldIndex, TIndex newIndex) {
    std::for_each(
        m_triangleMoveOperationNotifiers.begin(),
        m_triangleMoveOperationNotifiers.end(),
        [&oldIndex, &newIndex](std::function<void(TIndex&, TIndex&)> function) {
          function(oldIndex, newIndex);
        });
  }

  void notifySelectedCornerChange(CIndex oldCorner, CIndex newCorner) {
    std::for_each(m_selectedCornerChangeOperationNotifiers.begin(),
                  m_selectedCornerChangeOperationNotifiers.end(),
                  [&oldCorner, &newCorner](
                      std::function<void(CIndex&, CIndex&)> function) {
 ...
 private:
  std::list<std::function<void(TIndex&, TIndex&)>>
      m_triangleMoveOperationNotifiers;  // Must use a container that doesn't
                                         // invalidate iterators
  std::list<std::function<void(VIndex&, VIndex&)>>
      m_vertexMoveOperationNotifiers;  // Must use a container that doesn't
                                       // invalidate iterators
  std::list<std::function<void(CIndex&, CIndex&)>>
      m_selectedCornerChangeOperationNotifiers;  // Must use a container that
...
#pragma region STATSFINDING
 public:
  void findValenceHistogram() {
    ...
    std::for_each(beginCornerIterator(), endCornerIterator(),
                  [this, &valence](CIndex cIndex) { valence[v(cIndex)]++; });
...
#pragma region DEBUG
 public:
  void colorTriangles(const std::vector<TIndex>& triangleList, COLORS color) {
    ...
    std::for_each(
        triangleList.begin(), triangleList.end(),
        [this, &colorIndex](TIndex tIndex) { m_tm[tIndex] = colorIndex; });
  }
  ...
  void colorVertices(const std::vector<VIndex>& vertexList, COLORS color) {
    ...
    std::for_each(
        ...
        [this, &colorIndex](VIndex vIndex) { m_vm[vIndex] = colorIndex; });
  }
  ...
  void colorCorners(const std::vector<CIndex>& cornerList, COLORS color) {
    ...
    unsigned int colorIndex = m_pColorMap->getIndexForColor(color);
    std::for_each(
        cornerList.begin(), cornerList.end(),
        [this, &colorIndex](CIndex cIndex) { m_cm[cIndex] = colorIndex; });
  }
