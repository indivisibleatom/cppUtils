...
public:
  ...
  Point( const Point& other ) throw() : m_x( other.m_x ), m_y( other.m_y ), m_z( other.m_z ) {}
  ...
  Point( const Point& point, float a, const Vector<T>& I, const Vector<T>& J, const Point& other ) // Rotated point by 'a' around other in plane (I,J)
  {
  ...
  Point( const Point& p1, const Point& p2 ) : m_x( (p1.m_x + p2.m_x)/2 ), m_y( (p1.m_y + p2.m_y)/2 ), m_z( (p1.m_z + p2.m_z)/2 ) {}
  ...
  Point( const Point& point, float s, const Point& other ) throw() // point + s ( other - point )
  {
  ...
  Point( const Point& point, float s, const Vector<T>& v1,  float t, const Vector<T>& v2 ) throw() : m_x( point.x() + s * v1.x() + t * v2.x() ),
  ...
  template < class U >
  Point<U> quantizePoint( const Point<T>& minPoint, const Point<T>& maxPoint, int numBits ) const
  {
    Point<U> retVal;
  ...
  template <class T> friend Point<T> operator- ( const Point<T>& first, const Point<T>& second );
  template <class T> friend std::ostream& operator<< ( std::ostream& os, const Point<T>& point );
};
...
{
  Point<T> retVal;
  retVal.set( first.m_x - second.m_x, first.m_y - second.m_y, first.m_z - second.m_z );
  return retVal;
}
...
template <class T>
void deltaEncode( const std::vector< Point< T > >& inputPoints, std::vector< Point < T > >& outputPoints ) 
{
  Point< T >& prevPoint;
  std::for_each( inputPoints.begin(), inputPoints.end(), [ &inputPoints, &outputPoints ]( const Point< T >& point ) {
    if ( !outputPoints.empty() )
    {
      std::function< Point< T >( const Point< T >&, const Point< T >& ) > diffFunc = operator-;
